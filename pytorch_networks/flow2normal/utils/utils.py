...
sys.path.append('../..')
from api.utils import exr_saver, exr_loader
...
def create_grid_image(flows, outputs, labels, max_num_images_to_save=3):
    ...
    mask_invalid_pixels_rgb = torch.ones_like(flow_tensor)
    mask_invalid_pixels_rgb[mask_invalid_pixels] = 0

    images = torch.cat((flow_tensor, output_tensor_rgb, label_tensor_rgb, mask_invalid_pixels_rgb), dim=3)
