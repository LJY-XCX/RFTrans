...
from . import inference_models_rftrans, utils
...
class DepthToDepthCompletion(object):
    ...
    FOLDER_MAP = {
        ...
        'masked-output-depth': {
            'postfix': '-masked-depth.exr',
            'folder-name': 'masked-depth'
        },
        'masked-output-point-cloud': {
            'postfix': '-masked-pointcloud.ply',
            'folder-name': 'masked-point-cloud'
        },
        'output-flows':{
            'postfix': '-output-flow.flo',
            'folder-name': 'output-flows'
        },
        'output-rgb-flows':{
            'postfix': '-output-rgb-flow.png',
            'folder-name': 'output-flows'
        },
        'masks': {
            'postfix': '-mask.png',
    ...
    def __init__(
            self,
            rgb2flowWeightsFile,
            flow2normalWeightsFile,
            outlinesWeightsFile,
            masksWeightsFile='',
            rgb2flowModel='drn',
            flow2normalModel='simple_unet',
            ...
            maskinferenceHeight=512,
            maskinferenceWidth=512,
            normalsInferenceHeight=512,
            normalsInferenceWidth=512,
            outlinesInferenceHeight=512,
        ...
        self.masked_output_depth = None
        self.filtered_output_depth = None
        self.output_flows = None
        self.output_rgb_flow = None
        ...
        self.inferenceNormals = inference_models_rftrans.InferenceNormals(rgb2flowWeightsFile=rgb2flowWeightsFile,
                                                                    rgb2flowModel=rgb2flowModel,
                                                                    flow2normalWeightsFile=flow2normalWeightsFile,
                                                                    flow2normalModel=flow2normalModel,
                                                                    imgHeight=normalsInferenceHeight,
                                                                    imgWidth=normalsInferenceWidth)

        self.inferenceOutlines = inference_models_rftrans.InferenceOutlines(outlinesWeightsFile=outlinesWeightsFile,
        ...
        if masksWeightsFile:
            self.inferenceMasks = inference_models_rftrans.InferenceMasks(masksWeightsFile=masksWeightsFile,
                                                                  masksModel=masksModel,
                                                                  imgHeight=maskinferenceHeight,
                                                                  imgWidth=maskinferenceWidth)

    def estimate_normals_write_ptcloud(self, filename, xyz_points, height, width, calc_normal=True):
    ...
    def store_depth_completion_outputs(self, root_dir, files_prefix, min_depth=0.0, max_depth=1.5, calc_normal=True):
        ...
        masked_output_depth_dir = os.path.join(root_dir, self.FOLDER_MAP['masked-output-depth']['folder-name'])
        masked_output_ptcloud_dir = os.path.join(root_dir, self.FOLDER_MAP['masked-output-point-cloud']['folder-name'])
        ...
        output_flow_dir = os.path.join(root_dir, self.FOLDER_MAP['output-flows']['folder-name'])
        ...
        xyz_points, rgb_points = utils._get_point_cloud(self.input_image, self.depth_gt, self.fx, self.fy, self.cx,
                                                        self.cy)
        self.estimate_normals_write_ptcloud(gt_ptcloud_filename, xyz_points, self.outputImgHeight, self.outputImgWidth, calc_normal=calc_normal)
        ...
        xyz_points, rgb_points = utils._get_point_cloud(self.input_image, self.orig_input_depth, self.fx, self.fy,
                                                        self.cx, self.cy)
        self.estimate_normals_write_ptcloud(input_ptcloud_filename, xyz_points, self.outputImgHeight, self.outputImgWidth, calc_normal=calc_normal)

        masked_ptcloud_filename = ('{:09d}'.format(files_prefix) + self.FOLDER_MAP['masked-output-point-cloud']['postfix'])
        masked_ptcloud_filename = os.path.join(orig_input_ptcloud_dir, masked_ptcloud_filename)
        xyz_points, rgb_points = utils._get_point_cloud(self.input_image, self.orig_input_depth, self.fx, self.fy,
                                                        self.cx, self.cy)
        self.estimate_normals_write_ptcloud(masked_ptcloud_filename, xyz_points, self.outputImgHeight, self.outputImgWidth, calc_normal=calc_normal)
        ...
        utils.write_point_cloud(input_ptcloud_filename, self.input_image, self.input_depth, self.fx, self.fy, self.cx, self.cy)
        ...
        # Mask output depth with predicted mask and input depth (or 0)
        # self.output_depth[self.mask_predicted == 0] = 0
        ...
        masked_output_depth_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['masked-output-depth']['postfix']
        masked_output_depth_filename = os.path.join(masked_output_depth_dir, masked_output_depth_filename)
        utils.exr_saver(masked_output_depth_filename, self.masked_output_depth, ndim=3)
        ...
        masked_output_ptcloud_filename = ('{:09d}'.format(files_prefix) + self.FOLDER_MAP['masked-output-point-cloud']['postfix'])
        masked_output_ptcloud_filename = os.path.join(masked_output_ptcloud_dir, masked_output_ptcloud_filename)
        utils.write_point_cloud(masked_output_ptcloud_filename, self.surface_normals_rgb, self.masked_output_depth, self.fx, self.fy,
                                self.cx, self.cy)
        ...
        # Store Flows
        flow_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['output-flows']['postfix']
        flow_filename = os.path.join(output_flow_dir, flow_filename)
        utils.flow_saver(self.output_flows.cpu().numpy(), flow_filename)

        rgb_flow = utils.flow_to_rgb(self.output_flows.cpu().numpy())
        rgb_flow = rgb_flow.permute((1, 2, 0))
        rgb_flow_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['output-rgb-flows']['postfix']
        rgb_flow_filename = os.path.join(output_flow_dir, rgb_flow_filename)
        imageio.imwrite(rgb_flow_filename, rgb_flow)
        ...
        output_depth_rgb = utils.depth2rgb(self.output_depth,
                                           ...
                                           max_depth=max_depth,
                                           color_mode=COLOR_MAP)
        masked_depth_rgb = utils.depth2rgb(self.masked_output_depth,
                                           min_depth=min_depth,
                                           max_depth=max_depth,
                                           color_mode=COLOR_MAP)
        ...
        masked_depth_filename = '{:09d}'.format(files_prefix) + '-masked-output-depth-rgb.png'
        masked_depth_filename = os.path.join(masked_output_depth_dir, masked_depth_filename)
        imageio.imwrite(masked_depth_filename, masked_depth_rgb)
    ...
    def _modify_depth_delete_masks_normals(self, rgb_image, input_depth):
        # SURFACE NORMALS
        surface_normals, surface_normals_rgb, _ = self.inferenceNormals.runOnNumpyImage(rgb_image)
    ...
    def _complete_depth(self, input_image, input_depth, inertia_weight=1000, smoothness_weight=0.001, tangent_weight=1):
        ...
        # SURFACE NORMALS
        self.surface_normals, self.surface_normals_rgb, self.output_flows = self.inferenceNormals.runOnNumpyImage(input_image)
        self.output_flows = self.output_flows.squeeze(0)
    ...
    def depth_completion(self,
        ...
        self.masked_output_depth = self.output_depth.copy()
        self.masked_output_depth[self.mask_predicted == 0] = 0
        # Filter Output Depth
