...
from . import utils
from .modeling import deeplab, deeplab_masks, UNet
...
class InferenceNormals():
    ...
    def __init__(self, rgb2flowWeightsFile, flow2normalWeightsFile, rgb2flowModel='simple_unet', flow2normalModel='drn', imgHeight=288, imgWidth=512):
        ...
        # Create Model
        if rgb2flowModel == 'deeplab_resnet':
            print('Creating Deeplabv3-Resnet model for surface normals and loading checkpoint')
            self.r2f_model = deeplab.DeepLab(num_classes=2, backbone='resnet', sync_bn=True,
                                         freeze_bn=False)
        elif rgb2flowModel == 'drn':
            print('Creating DRN model for normals and loading checkpoint')
            self.r2f_model = deeplab.DeepLab(num_classes=2, backbone='drn', sync_bn=True,
                                         freeze_bn=False)  # output stride is 8 for drn
        else:
            raise NotImplementedError(
                'Model may only be "unet" or "deeplab_resnet". Given model is: {}'.format(rgb2flowModel))

        if flow2normalModel == 'simple_unet':
            self.f2n_model = UNet(n_channels=3, n_classes=3)
        else:
            raise NotImplementedError(
                'Model may only be "unet" or "deeplab_resnet". Given model is: {}'.format(flow2normalModel))

        # Load Checkpoint and its data (weights file)
        if not os.path.isfile(rgb2flowWeightsFile):
            raise ValueError('Invalid path to the given weights file in config. The file "{}" does not exist'.format(
                normalsWeightsFile))
        if not os.path.isfile(flow2normalWeightsFile):
            raise ValueError('Invalid path to the given weights file in config. The file "{}" does not exist'.format(
                normalsWeightsFile))

        r2f_checkpoint = torch.load(rgb2flowWeightsFile, map_location='cpu')
        f2n_checkpoint = torch.load(flow2normalWeightsFile, map_location='cpu')
        self.r2f_model.load_state_dict({k.replace('module.', ''): v for k, v in r2f_checkpoint['model_state_dict'].items()})
        self.f2n_model.load_state_dict({k.replace('module.', ''): v for k, v in f2n_checkpoint['model_state_dict'].items()})

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.r2f_model = self.r2f_model.to(self.device)
        self.f2n_model = self.f2n_model.to(self.device)
        self.r2f_model.eval()
        self.f2n_model.eval()
    ...
    def _normal_to_rgb(self, normals_to_convert):
        ...
        '''
        if normals_to_convert.min() > 0:
            camera_normal_rgb = (normals_to_convert * 255).astype(np.uint8)
        else:
    ...
    def runOnNumpyImage(self, img):
        ...
        with torch.no_grad():
            ...
            flow_outputs = self.r2f_model(inputs)

            f2n_inputs = torch.zeros(1, 3, img.shape[0], img.shape[1])
            f2n_inputs[:, :2, :, :] = flow_outputs
            f2n_inputs = f2n_inputs.type(torch.float32).to(self.device)

            normal_vectors = self.f2n_model(f2n_inputs)

            normal_vectors_norm = nn.functional.normalize(normal_vectors.double(), p=2, dim=1)

            surface_normals = normal_vectors_norm.squeeze(0).cpu().numpy()
        ...
        return surface_normals_rotated, surface_normals_rgb, flow_outputs


class InferenceFlow2Normal():

    def __init__(self, flow2normalWeightsFile, flow2normalModel='simple_unet', imgHeight=288, imgWidth=512):
        '''Class to run Inference of the normals Prediction Model
        ...
        checkpoint = torch.load(flow2normalWeightsFile, map_location='cpu')
