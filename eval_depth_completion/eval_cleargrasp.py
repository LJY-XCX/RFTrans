...
from api import depth_completion_api_cleargrasp
...
if __name__ == '__main__':
    ...
    depthcomplete = depth_completion_api_cleargrasp.DepthToDepthCompletion(normalsWeightsFile=config.normals.pathWeightsFile,
    ...
    for dataset in config.files:
        EXT_COLOR_IMG = ['.jpg', '.png']  #'-rgb.jpg' - includes normals-rgb.jpg
        EXT_DEPTH_IMG = ['.png']
        EXT_DEPTH_GT = ['.png']
        EXT_MASK = ['.png']
    ...
    for i in range(len(rgb_file_list)):
        ...
        try:
            output_depth, filtered_output_depth = depthcomplete.depth_completion(
                ...
                dilate_mask=True)
        except depth_completion_api_cleargrasp.DepthCompletionError as e:
